...
{
    /// <summary>
    /// https://stackoverflow.com/a/3504251/221018
    /// </summary>
    public static class FileLockInfo
    {
        public static string LockDesc(string path)
        {
            try
            {
                List<Process> processes = GetProcessesLockingFile(path);
                string desc = string.Join(", ", processes.Select(x=>x.ProcessName));
                return (desc == string.Empty) ? string.Empty : $"Locked ({desc})";
            }
            catch (Exception)
            {
